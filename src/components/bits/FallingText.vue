<!--
  FallingText — sourced from vue-bits (DavidHDev/vue-bits), the Vue port of React Bits.
  License: MIT + Commons Clause v1.0 — https://github.com/DavidHDev/vue-bits/blob/main/LICENSE.md
  Adapted for Setobox Home: highlighted-word color changed to the violet token.
-->
...
const createTextHTML = () => {
  ...
  const newHTML = words
    .map((word) => {
      ...
      return `<span class="inline-block mx-[2px] select-none ${isHighlighted ? "text-violet-300 font-bold" : ""}">${word}</span>`;
    })
    .join(" ");
