<!--
  LogoLoop — sourced from vue-bits (DavidHDev/vue-bits), the Vue port of React Bits.
  License: MIT + Commons Clause v1.0 — https://github.com/DavidHDev/vue-bits/blob/main/LICENSE.md
  Adapted for Setobox Home: scaleOnHover references props explicitly; scale step adjusted; fade fallback color set to the void token.
-->
<template>
  <div
    ref="containerRef"
      ...
      >
        <li
          ...
          :class="[
            ...
            props.scaleOnHover && 'overflow-visible group/item',
          ]"
          role="listitem"
        >
          <slot
          ...
          <template v-else>
            ...
            >
              <LogoContent :item="item" :scale-on-hover="props.scaleOnHover" />
            </a>
            <LogoContent v-else :item="item" :scale-on-hover="props.scaleOnHover" />
          </template>
        </li>
      </ul>
    </div>
